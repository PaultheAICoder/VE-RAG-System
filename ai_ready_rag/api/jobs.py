...
from ai_ready_rag.db.models import ReindexJob, User, WarmingQueue
...
def _find_job(db: Session, job_id: str) -> tuple[str, WarmingQueue | ReindexJob | None]:
    """Find a job by ID across WarmingQueue and ReindexJob tables.

    Returns (type, job) tuple. Type is "cache_warming" or "reindex".
    """
    warming_job = db.query(WarmingQueue).filter(WarmingQueue.id == job_id).first()
    if warming_job:
        return "cache_warming", warming_job
...
def _warming_to_status(job: WarmingQueue) -> JobStatusResponse:
    """Convert WarmingQueue to unified JobStatusResponse."""
    return JobStatusResponse(
        job_id=job.id,
        type="cache_warming",
        status=job.status,
        progress=JobProgress(
            total=job.total_queries or 0,
            processed=job.processed_queries or 0,
            failed=job.failed_queries or 0,
        ),
        result_summary=(
            f"{job.processed_queries}/{job.total_queries} queries processed"
...
async def get_job_status(
    ...
    Queries WarmingQueue and ReindexJob tables by ID.
...
async def stream_job_progress(
    ...
    async def event_generator():
        ...
        while True:
            ...
            try:
                if job_type == "cache_warming":
                    current = poll_db.query(WarmingQueue).filter(WarmingQueue.id == job_id).first()
                    if current:
                        response = _warming_to_status(current)
                else:
...
async def cancel_job(
    ...
    For warming jobs: sets is_cancel_requested flag.
    ...
    if job_type == "cache_warming":
        ...
        db.commit()
        logger.info(f"Admin {current_user.email} cancelled warming job {job_id}")
        return JobCancelResponse(
            job_id=job_id,
            cancelled=True,
            message="Cancel requested for warming job.",
        )
    else:
        job.status = "aborted"
        db.commit()
