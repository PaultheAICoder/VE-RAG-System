...
# =============================================================================
# Warming Queue Fixtures
# =============================================================================


@pytest.fixture(scope="function")
def warming_queue_dir(tmp_path):
    """Create temporary directory structure for warming queue tests.

    Returns:
        Path to the queue directory with jobs/, quarantine/, and archive/ subdirs.
    """
    queue_dir = tmp_path / "warming_queue"
    queue_dir.mkdir()
    (queue_dir / "jobs").mkdir()
    (queue_dir / "quarantine").mkdir()
    (queue_dir / "archive").mkdir()
    return queue_dir


@pytest.fixture(scope="function")
def warming_service(warming_queue_dir):
    """Create WarmingQueueService instance with temp directory.

    Note: This service is file-based only, no database session needed.
    """
    from ai_ready_rag.services.warming_queue import WarmingQueueService

    return WarmingQueueService(
...
@pytest.fixture(scope="function")
def warming_service_with_archive(warming_queue_dir):
    """Create WarmingQueueService with archiving enabled."""
    from ai_ready_rag.services.warming_queue import WarmingQueueService
...
@pytest.fixture(scope="function")
def sample_job(warming_service):
    """Create a sample warming job for tests."""
    return warming_service.create_job(
        queries=["What is the vacation policy?", "How do I request PTO?"],
